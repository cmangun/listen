...
interface ThemeContextProps {
   ...
   * @param color
   */
  changeComponentSkin: (componentName: ComponentNameTypes, color: ComponentSkinTypes) => void
...
const Theme: React.FC<ThemeProps> = (props) => {
  ...
  const changeComponentSkin = (componentName: ComponentNameTypes, color: ComponentSkinTypes) => {
  ...
  const replaceClassName = (className: string) => {
    ...
    if (isRTL) {
      classList.forEach((cl, index) => {
        ...
        classList[index] = isPrefix ? getReversePrefix(cl) : isSuffix ? getReverseSuffix(cl) : cl
      })
    }
