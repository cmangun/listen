...
interface TabProps extends React.HTMLAttributes<Omit<HTMLUListElement, 'role'>> {
  ...
  useEffect(() => {
    ...
    delay(1).then(() => {
      if (current) {
        ...
        const handler = (event: MouseEvent) => setLineProps(event.currentTarget as HTMLElement)
