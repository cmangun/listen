        ...
        <div className={replaceClassName("dropdown ms-3 ms-sm-4")}>
          ...
          >
            <div className="avatar__image">
              <img src={currentUser.cover || "/placeholder.svg"} width={128} height={128} alt="Profile picture" />
            </div>
          ...
          >
            <div className="py-2 px-3 avatar avatar--lg">
              <div className="avatar__image">
                <img src={currentUser.cover || "/placeholder.svg"} width={128} height={128} alt={currentUser.name} />
              </div>
            ...
            {OPTIONS.map((option, index) => (
              <Link to={option.path} key={index} className="dropdown-item d-flex align-items-center">
                {<option.icon size={20} />}
                <span className={replaceClassName("ms-2")}>{user[option.name]}</span>
              </Link>
            ))}
            <div className="dropdown-divider"></div>
            <a role="button" className="dropdown-item d-flex align-items-center text-danger" onClick={logout}>
