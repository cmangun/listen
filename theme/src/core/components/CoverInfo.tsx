...
const CoverInfo: React.FC<Props> = ({ data }) => {
      ...
      list && (
        <p className='mb-2'>
          ...
          {list.map((item: any, index: number) => (
            <Link key={item.id} className='text-dark fw-medium' to={'/music/artist/' + item.id}>
              {item.name}
              {index !== list.length - 1 ? ', ' : ''}
            </Link>
          ))}
  ...
  const getSectionInfoList = () => {
    return location.pathname.includes('album') ? (
      <>
        <li>{t('sidebar.albums')}</li>
        <li>
          {data.tracks.length} {t('tracks')}
        </li>
      </>
    ) : (
      <>
        ...
        {data.totalAlbums && (
          <li>
            {data.totalAlbums} {t('sidebar.albums')}
          </li>
        )}
        {data.totalTracks && (
          <li>
            {data.totalTracks} {t('tracks')}
          </li>
        )}
      </>
    )
      ...
      <div className='row'>
        ...
        <div className='col-md-8 mt-5 mt-md-0'>
          <div className='d-flex align-items-center flex-wrap'>
            ...
            <TrackDropdown
              className={replaceClassName('dropstart d-inline-flex ms-auto')}
              data={data}
              queue={!data.tracks}
              play
              playlist
          ...
          {(data.artists || data.composers || data.lyricists || data.directors) && (
            <div className='mt-3'>
          ...
          <ul className='info-list mt-5'>
            <li>
              <div className='d-flex align-items-center'>
                {data.tracks ? (
                  ...
                  <>
                    <PlayButton primaryButton track={data} />
            ...
            <li>
              ...
              >
                ...
                <span className={replaceClassName('ps-2 fw-semibold')}>{data.favorites}</span>
              </a>
            </li>
            {data.downloads && (
              <li>
                ...
                >
                  <RiDownload2Line />
                  <span className={replaceClassName('ps-2 fw-semibold')}>{data.downloads}</span>
                </a>
              </li>
            )}
            <li>
              <span className='text-dark d-flex align-items-center'>
                ...
                <span className={replaceClassName('ps-2 fw-semibold')}>{data.rating}</span>
              </span>
            </li>
          </ul>
          ...
          {data.lyrics && (
            <div className='mt-5'>
              <span className='d-block text-dark fs-6 fw-semibold mb-3'>{t('lyrics')}</span>
