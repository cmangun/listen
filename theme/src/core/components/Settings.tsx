...
import { ComponentNameTypes, ComponentSkinTypes, ThemeModeTypes } from '../types'
...
const Settings: React.FC = () => {
  ...
  useEffect(() => setIsShowInfo(!location.pathname.startsWith('/music')), [location.pathname])
   ...
   * @returns
   */
  const getActive = (component: ComponentNameTypes, color: ComponentSkinTypes) => {
   ...
   */
  const toggleSettings = useCallback(() => settingRef.current?.classList.toggle('show'), [])

  useEventListener('click', handleDOMClick, documentRef as RefObject<Document>)
        ...
        <Scrollbar className='flex-1'>
          <div className='setting__body'>
            ...
            <div className='setting__theme'>
              {themeOptions.map((option) => (
                <a
                  ...
                  className={classNames('setting__option', theme === option && 'active')}
                >
                  <img src={'/images/misc/' + option + '.svg'} width={94} height={45} alt='' />
            ...
            <div className='setting__rtl'>
              <label htmlFor='rtl' className='setting__title mb-0'>
                Right-To-Left
              </label>
        ...
        {isShowInfo && (
          <div className='setting__info'>
            ...
            <span className='flex-grow-1'>
              You can observe the color change effect in the header, sidebar, and player components
              on the inner pages.
            </span>
          </div>
        )}
